...
{	
	for (std::map<std::string, CollisionBox>::iterator it = this->collisionBox.begin(); it != this->collisionBox.end(); ++it) {
		float x = (this->getPos().x + this->getSize().x / 2) - (this->collisionBox[it->first].getSize().x / 2);
		float y = (this->getPos().y + this->getSize().y) - (this->collisionBox[it->first].getSize().y);
...
void Creature::setCollisionBox(std::map<std::string, CollisionBox> colls)
{
	for (std::map<std::string, CollisionBox>::iterator it = colls.begin(); it != colls.end(); ++it) {
		this->collisionBox[it->first].draw();
	}
